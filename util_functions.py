...
def print_trip_results(best_tuesdays: List[Trips], best_wednesdays: List[Trips], best_overnight: List[Trips], debug_hashes = False):
    def print_trip_section(title: str, trips: List[Trips]):
        ...
        for i, trip in enumerate(trips, 1):
            ...
            print(f"Outbound: {trip.outbound.date} - {trip.outbound.departure_arrival}: £{trip.outbound.cost:.2f} ({trip.outbound.travel_time_str})  {hash_outbound}")
            print(f"Date Return: {trip.return_trip.date} - {trip.return_trip.departure_arrival}: £{trip.return_trip.cost:.2f} ({trip.return_trip.travel_time_str}) {hash_return}")
            print(f"Total cost: £{trip.cost():.2f}")

    print_trip_section(f"{'Best' if len(best_tuesdays) == 1 else 'Other Options' } Tuesday Same-Day Trips", best_tuesdays)
    print_trip_section(f"{'Best' if len(best_wednesdays) == 1 else 'Other Options'} Wednesday Same-Day Trips", best_wednesdays)
    print_trip_section(f"{'Best' if len(best_overnight) == 1 else 'Other Options'} Overnight Trips", best_overnight)

def create_date_pairs(date):
    """Create pairs of dates for analysis."""
    # Get all Tuesdays and Wednesdays
    # Check date is not in the past
    now = datetime.now()
    dates = _get_tuesdays_wednesdays(date.year, date.month, date.day)
    pairs = []
...
def _get_tuesdays_wednesdays( start_year, start_month, start_day):
    ...
    end_date = datetime(year, int(start_month), last_day)

    # Start from the first day of the month (or current date if it's later)
    current_date = datetime(start_date.year, start_date.month, max(1, start_date.day))
